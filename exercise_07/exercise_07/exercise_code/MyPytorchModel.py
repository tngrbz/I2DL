...
class MyPytorchModel(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.layers = nn.ModuleList([nn.Linear(in_size, n_hidden), nn.ReLU(), nn.Linear(n_hidden, n_hidden), 
                                     nn.ReLU(), nn.Linear(n_hidden, num_classes)])
...
class CIFAR10DataModule(nn.Module):
    ...
    def prepare_data(self, stage=None, CIFAR_ROOT="../datasets/cifar10"):
        ...
        ########################################################################
        my_transform = transforms.Compose([transforms.Resize((32, 32)), transforms.ToTensor(), transforms.Normalize(mean, std)])
        pass
        ...
        test_val_transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean, std)])
        ...
        if self.opt['loading_method'] == 'Image':
            ...
            cifar_complete_train_val = torchvision.datasets.ImageFolder(root=CIFAR_ROOT, transform=test_val_transform)
